...
namespace LegalLabMaui.Models.Design
{
	...
	public class DesignModel : ConnectionSensitiveModel, IPartsModel, ITranslatable
	{
		...
		/// <summary>
		/// Sets the active contract and refreshes all bound properties.
		/// </summary>
		public async Task SetContract(Contract Contract)
		{
			...
			foreach (Parameter Parameter in this.contract.Parameters)
			{
				if (Parameter is BooleanParameter BP)
					ParameterInfo = new BooleanParameterInfo(this.contract, BP, this, this.parameters);
				else if (Parameter is NumericalParameter NP)
					ParameterInfo = new NumericalParameterInfo(this.contract, NP, this, this.parameters);
				else if (Parameter is StringParameter SP)
					ParameterInfo = new StringParameterInfo(this.contract, SP, this, this.parameters);
				else if (Parameter is DateParameter DP)
					ParameterInfo = new DateParameterInfo(this.contract, DP, this, this.parameters);
				else if (Parameter is DateTimeParameter DTP)
					ParameterInfo = new DateTimeParameterInfo(this.contract, DTP, this, this.parameters);
				else if (Parameter is TimeParameter TP)
					ParameterInfo = new TimeParameterInfo(this.contract, TP, this, this.parameters);
				else if (Parameter is DurationParameter DrP)
					ParameterInfo = new DurationParameterInfo(this.contract, DrP, this, this.parameters);
				else if (Parameter is CalcParameter CP)
					ParameterInfo = new CalcParameterInfo(this.contract, CP, this, this.parameters);
				else if (Parameter is RoleParameter RP)
				{
					RoleReferenceParameterList.Add(new RoleReferenceParameterInfo(this.contract, RP, this, this.roleReferenceParameters));
					continue;
				}
				else if (Parameter is ContractReferenceParameter CRP)
				{
					ContractReferenceParameterList.Add(new ContractReferenceParameterInfo(this.contract, CRP, this, this.contractReferenceParameters));
					continue;
				}
				else if (Parameter is GeoParameter GP)
					ParameterInfo = new GeoParameterInfo(this.contract, GP, this, this.parameters);
				else
					continue;
		...
		/// <summary>
		/// English language ISO-639-1 record array.
		/// </summary>
		...
		private Task NetworkModel_OnStateChanged(object? Sender, XmppState NewState)
		{
			AppService.UpdateGui(() =>
			{
				try
				{
					this.Connected = NewState == XmppState.Connected;
					this.propose.RaiseCanExecuteChanged();
				}
				catch (Exception ex)
				{
		...
		/// </summary>
		public NetworkModel? Network => this.networkModel ?? AppService.NetworkModel;
		...
		{
			public ITranslatable Object = null!;
		...
		/// <summary>
		/// If contract must be signed after a specific time.
		/// </summary>
		...
		public XmlElement? ForMachines
		{
			...
			set
			{
				...
				this.forMachinesLocalName.Value = value?.LocalName ?? string.Empty;
				this.forMachinesNamespace.Value = value?.NamespaceURI ?? string.Empty;
			}
		}
		...
		/// </summary>
		public string ForMachinesLocalName => this.forMachinesLocalName.Value;

		/// <summary>
		/// Namespace of top element of Machine-Readable XML in the contract
		/// </summary>
		public string ForMachinesNamespace => this.forMachinesNamespace.Value;
		...
		/// <summary>
		/// Human-readable markdown (editable source)
		/// </summary>
		...
		/// <summary>
		/// Rendered human-readable text (plain text or HTML result).
		/// </summary>
			...
			AppService.UpdateGui(async () =>
			{
				try
				{
					...
					else
					{
						...
						string Markdown = await Text.GenerateMarkdown(this.Contract, MarkdownType.ForRendering);
						this.HumanReadableText = Markdown ?? string.Empty;
					}
				}
				catch (Exception ex)
				{
		...
		public async Task ValidateParameters()
		{
			...
			foreach (ParameterInfo P in this.AllParameterInfos)
			{
				bool Valid = await P.ValidateParameter(Variables);
				if (!Valid)
					Ok = false;
			}
		...
		/// <summary>
		/// Called when a parameter value changes in the designer.
		/// </summary>
		/// <param name="Parameter">Updated parameter.</param>
		public void ParameterValueChanged(ParameterInfo Parameter)
		{
			_ = AppService.UpdateGui(async () =>
			{
				await this.ValidateParameters();
		...
		/// <summary>
		/// Role reference Parameters defined in the contract.
		/// </summary>
		...
		/// <summary>
		/// Contract reference Parameters defined in the contract.
		/// </summary>
		...
		/// <summary>
		/// An array of all parameter infos across all parameter lists.
		/// </summary>
		...
		/// <summary>
		/// An array of all underlying Parameter objects.
		/// </summary>
		...
		private async Task ExecuteAddNumericParameter(string Name, decimal? Value, params string[] MarkdownDescription)
		{
			...
			this.AddParameter(new NumericalParameterInfo(this.contract, NP, this, this.parameters));
		}
		...
		private async Task ExecuteAddStringParameter(string Name, string RegEx, int? MaxLength, params string[] MarkdownDescription)
		{
			...
			this.AddParameter(new StringParameterInfo(this.contract, SP, this, this.parameters));
		}
		...
		private async Task ExecuteAddBooleanParameter(string Name, bool? Value, params string[] MarkdownDescription)
		{
			...
			this.AddParameter(new BooleanParameterInfo(this.contract, BP, this, this.parameters));
		}
		...
		private async Task ExecuteAddDateParameter(string Name, DateTime? Value, params string[] MarkdownDescription)
		{
			...
			this.AddParameter(new DateParameterInfo(this.contract, DP, this, this.parameters));
		}
		...
		public async Task ExecuteAddDateTimeParameter()
		{
			...
			this.AddParameter(new DateTimeParameterInfo(this.contract, DP, this, this.parameters));
		}
		...
		private async Task ExecuteAddTimeParameter(string Name, TimeSpan? Value, params string[] MarkdownDescription)
		{
			...
			this.AddParameter(new TimeParameterInfo(this.contract, TP, this, this.parameters));
		}
		...
		public async Task ExecuteAddDurationParameter()
		{
			...
			this.AddParameter(new DurationParameterInfo(this.contract, DP, this, this.parameters));
		}
		...
		public async Task ExecuteAddCalcParameter()
		{
			...
			this.AddParameter(new CalcParameterInfo(this.contract, CP, this, this.parameters));
		}
		...
		public async Task ExecuteAddRoleReference()
		{
			...
			this.AddRoleReferenceParameter(new RoleReferenceParameterInfo(this.contract, RP, this, this.roleReferenceParameters));
		}
		...
		public async Task ExecuteAddContractReference()
		{
			...
			this.AddContractReferenceParameter(new ContractReferenceParameterInfo(this.contract, CRP, this, this.contractReferenceParameters));
		}
		...
		public async Task ExecuteAddGeoParameter()
		{
			...
			this.AddParameter(new GeoParameterInfo(this.contract, GP, this, this.parameters));
		}
			...
			try
			{
				if (QueryUser)
				{
					bool Confirmed = await AppService.MessageBox(
						"Are you sure you want to clear the form and lose all data that has not been saved?",
						"Confirm",
						IsQuestion: true);
		...
		private async Task ExecuteLoadContract()
		{
			try
			{
				...
				{
					PickerTitle = "Load Smart Contract",
		...
		private async Task ExecuteSaveContract()
		{
			try
			{
				string Xml = this.contract.ToXml();
				(string Pretty, XmlElement _) = Xml.ToPrettyXml();

				string SafeName = string.IsNullOrEmpty(this.ContractId)
					? "contract.xml"
					: this.ContractId.Replace(":", "_").Replace("/", "_") + ".xml";

				string TargetPath = Path.Combine(FileSystem.AppDataDirectory, SafeName);

				await System.IO.File.WriteAllTextAsync(TargetPath, Pretty);

				AppService.SuccessBox("Contract saved to: " + TargetPath);
			}
			catch (Exception ex)
			{
				AppService.ErrorBox(ex.Message);
		...
		private async Task ExecuteImportContract()
		{
			try
			{
				FileResult Result = await FilePicker.PickAsync(new PickOptions()
				{
					PickerTitle = "Import from Microsoft Word",
					FileTypes = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>>
					{
				...
				try
				{
					...
					using (FileStream dst = System.IO.File.Create(TempFileName))
					{
				...
				this.ContractId = Path.GetFileNameWithoutExtension(Result.FileName);
		...
		private async Task ExecuteAddLanguage()
		{
			// Navigate to the AddLanguage page; result handled via messaging or query parameters.
			await Shell.Current.GoToAsync(nameof(AddLanguagePage));
		}

		/// <summary>
		/// Called by AddLanguagePage after the user confirms a language selection.
		/// </summary>
		/// <param name="SelectedLanguage">BCP-47 / ISO-639-1 language code.</param>
		public void OnLanguageAdded(string SelectedLanguage)
		{
			if (string.IsNullOrEmpty(SelectedLanguage))
				return;

			this.Languages = this.Languages.ToCodes().Append(SelectedLanguage).ToIso639_1();
			this.Language = SelectedLanguage.ToLower();
		}
		...
		private async Task ExecuteRemoveLanguage()
		{
			string Language = this.Language;

			bool Confirmed = await AppService.MessageBox(
				"Are you sure you want to remove language " + Language + " from the contract?",
				"Confirm",
				IsQuestion: true);
		...
		private async Task ExecuteProposeContract()
		{
			try
			{
				LegalModel LegalModel = this.networkModel?.Legal;
				if (LegalModel is null)
				{
					AppService.ErrorBox("Not connected to a legal component.");
					return;
				}

				bool Confirmed = await AppService.MessageBox(
					"Are you sure you want to propose the designed template to " + LegalModel.Contracts.ComponentAddress + "?",
					"Confirm",
					IsQuestion: true);
				...
				AppService.SuccessBox("Template successfully proposed. The template ID, which has been copied to the clipboard, is: " + Contract.ContractId);
				await Clipboard.SetTextAsync(Contract.ContractId);

				StringBuilder Url = new();

				Url.Append("https://");
				Url.Append(this.networkModel.XmppServer);
				Url.Append("/Contract.md?ID=");
				Url.Append(Contract.ContractId);

				AppService.OpenUrl(new Uri(Url.ToString()));
			}
			catch (Exception ex)
			{
				AppService.ErrorBox(ex.Message);
