...
namespace LegalLabMaui.Models.Legal
{
	...
	public class LegalModel : PersistedModel, IDisposable
	{
		...
		private Task Contracts_PetitionForSignatureReceived(object Sender, SignaturePetitionEventArgs e)
		{
			...
			{
				bool Confirmed = await AppService.MessageBox(Question.ToString(), "Petition received", true);
			...
			AppService.UpdateGui(async () =>
			{
				if (!this.AutoSignProposals)
				{
					bool Confirmed = await AppService.MessageBox(
						"You have received a proposal to sign a contract as " +
						e.Role + ", with the following message:\r\n\r\n" + e.MessageText +
						"\r\n\r\nDo you want to review the contract?",
						"Confirm", true);
		...
		public static async Task<string> GetCategory(Contract Contract)
		{
			...
			string Language = AppService.DesignModel?.Language ?? Translator.DefaultLanguageCode;
		...
		private Task Contracts_ClientMessage(object Sender, ClientMessageEventArgs e)
		{
			...
			AppService.MessageBox(sb.ToString(), "Identity Application Validation Result").Wait();
		...
		private async Task LoadTemplate(string TemplateName, Dictionary<CaseInsensitiveString, object> PresetValues)
		{
			...
			try
			{
				...
				await this.currentContract.PopulateParameters(PresetValues);
			}
			catch (Exception ex)
			{
